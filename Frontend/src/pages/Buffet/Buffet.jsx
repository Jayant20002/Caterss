...
const Buffet = ({catering}) => {
  ...
  const fetchBuffetData = async () => {
    try {
      const response = await axios.get("http://localhost:5001/buffet");
  ...
  const handlePackageChange = (e) => {
    ...
    // Map the package names to match the Packages component
    let packageName;
    switch(packageValue) {
      case "Silver Package":
      ...
      case "Diamond Package":
        ...
        break;
      case "Cocktail Package":
        packageName = "Cocktail Diamond Exotic";
        break;
      default:
        packageName = packageValue;
    }
    
    // Get user confirmation for direct checkout
    Swal.fire({
      title: 'Proceed to checkout?',
      text: "Would you like to directly proceed to checkout or customize your package first?",
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#4CAF50',
      cancelButtonColor: '#6B7280',
      confirmButtonText: 'Go to Checkout',
      cancelButtonText: 'Customize Package'
    }).then((result) => {
      if (result.isConfirmed) {
        // Direct to checkout
        const price = packageName === "Silver Package" ? 800 : 
                     packageName === "Gold Package" ? 1200 : 
                     packageName === "Diamond Package" ? 1500 : 
                     packageName === "Cocktail Diamond Exotic" ? 1800 : 1000;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:5001/buffet", formData);
 ...
 const getCurrentDate = () => {
  ...
  return (
    <div>
      ...
      {catering === "Event Catering" && <div className="col categories-item">
